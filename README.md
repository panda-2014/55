...
import queue
import cv2
def start_ffmpeg_stream(rtsp_url, frame_queue: queue.Queue):
    cmd = [
        'ffmpeg', '-rtsp_transport', 'tcp',
        '-stimeout', '5000000',
        '-rtbufsize', '100M',
...
threading.Thread(target=start_ffmpeg_stream, args=(url, q), daemon=True).start()




if frame_queue.qsize() > 20:  # 延迟严重
    restart_with_lower_resolution()
elif frame_queue.qsize() == 0:
    log_warning("Input starvation detected")
